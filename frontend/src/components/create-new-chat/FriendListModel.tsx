...
const FriendListModel = () => {
  ...
  const handleAddConversation = async (firendId: string) => {
    await createConversation("direct", "", [firendId]);
  };
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto">
          ...
          {friends.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              <User className="size-12 mx-auto mb-3 opacity-50" /> Chưa có bạn
              bè, thêm bạn vào để trò chuyện
            </div>
          )}
        </div>
      </div>
